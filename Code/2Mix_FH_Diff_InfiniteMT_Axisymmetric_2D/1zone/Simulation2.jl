...
@inline function evolve_phi!(ϕ_temp, ϕ, ∇ϕ, Δϕ, μ, Δμ, Nx, Nz, k, h, R, χ, dt, dx, M, a)
    @cuda threads = block_dim blocks = grid_dim kernel_comp_∇_Δ!(Δϕ, ∇ϕ, ϕ, Nx, Nz, h, R, dx)
...
#Initialization of a film.
function kernel_film_init!(ϕ, ϕ_dilute, ϕ_dense, film_width, dx, A, λ, Nx)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y

    @inbounds begin

        ϕ[i,j] = (j-1.0)*dx > film_width + A*CUDA.sin(λ*2*pi*i*dx/Nx) ? ϕ_dilute : ϕ_dense
...
@inline function film_init!(ϕ, ϕ_dilute, ϕ_dense, film_width, dx, A, λ, Nx)
    @cuda threads = block_dim blocks = grid_dim kernel_film_init!(ϕ, ϕ_dilute, ϕ_dense, film_width, dx, A, λ, Nx)
end

#Simulation ###############################################################################

#Initialization
ϕ = CUDA.ones(Tf, Nx, Nz)
film_init!(ϕ, ϕ_dilute, ϕ_dense, film_width, dx, A, λ, Nx)
...
for t=0:NΔt
    ...
    evolve_phi!(ϕ_temp, ϕ, ∇ϕ, Δϕ, μ, Δμ, Nx, Nz, k, h, R, χ, dt, dx, M, a)
