...
function kernel_diffusion!(ϕn, ϕ, ϕ0, ϕa, ϕb, μ, β, k, M, dt, dx, x1, x2, y1, y2, xm, ym, is_in, kr)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y
...
gpukernel_diffusion = @cuda launch = false kernel_diffusion!(ϕ_temp, ϕ, ϕ0, ϕa, ϕb, μ, β, k, M, dt, dx, xl, xr, yb, yt, xm, ym, is_in, kr)
...
CUDA.@time begin
    ...
    for t = 1:NΔt
        ...
        for i = 1:Ndtd
            ...
            gpukernel_comp_derivative(Δϕ, ∇ϕ, ϕ, ϕ0, a, a0, atip, lzone, tail, lzone0, dx, xl, xr, yb, yt, xm, ym; threads=a2D_block, blocks=a2D_grid)
            gpukernel_comp_μ(μ, ϕ, Δϕ, ϕa, ϕb, β, k; threads=a2D_block, blocks=a2D_grid)
            gpukernel_diffusion(ϕ_temp, ϕ, ϕ0, ϕa, ϕb, μ, β, k, M, dtd, dx, xl, xr, yb, yt, xm, ym, is_in, kr; threads=a2D_block, blocks=a2D_grid)
            copyto!(ϕ, ϕ_temp)
            if any(isnan, ϕ)
        ...
        if t % prin == 0 && t >= prin0
            println(Dates.now())
            println(t)
            save(string(dird, "data_", @sprintf("%08i", t), ".jld"), "density", Array(ρ), "velocity", Array(v), "PF", Array(ϕ), "tip", xm)
            GC.gc()
            CUDA.memory_status()
        end
